    ...
    def init_sprites(self):
        ...
        for i in range(len(LEVEL_1_FLOOR)):
            if LEVEL_1_FLOOR[i] == 0:
                self.floors.add(Floor(i*2*WIDTH, HEIGHT/4*3,
                                2*WIDTH, 10, (255, 255, 255)))
                ground_obstacles = LEVEL_1_GROUND_OBSTACLES[i]
                for j in range(len(ground_obstacles)):
                    if ground_obstacles[j] == 1:
                        self.obstacle.add(
                            Obstacle(i*2*WIDTH + j*2*WIDTH/10, HEIGHT/4*3, WIDTH/16, WIDTH/16))
            else:
                self.lavas.add(Floor(i*2*WIDTH, HEIGHT/4*3,
                               2*WIDTH, 10, (50, 50, 50)))
                ...
                for j in range(len(air_floors)):
                    ...
                    if value > 0:
                        self.floors.add(Floor(
                            i*2*WIDTH + j*2*WIDTH/10, HEIGHT/4*3 - HEIGHT/10*value, 2*WIDTH/10, 10, (50, 50, 50)))
